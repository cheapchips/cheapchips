...
export default function useContract(
    ...
    const getContract = async () => {
        ...
        switch (contractName) {
            case "ChipStable":
                ctrct = await ChipStable__factory.connect("0xCb121efF8eAdB7Ab2CaA0660cFD02e5BE4C946b6", provider)
                break;
            case "ChipsJackpot":
                ctrct = await ChipsJackpot__factory.connect("0xf082812C3De7a8d5014f1F748bb75046F6143A53", provider)
            console.log(ctrct)
            await setContract(await ctrct)
        }
    }
